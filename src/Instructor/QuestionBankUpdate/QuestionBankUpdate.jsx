...
function QuestionBankUpdate() {
  ...
  useEffect(() => {
    if (moduleid) {
      axios
        .get(
          `${process.env.REACT_APP_API_URL}quiz/updatequestionbank/${moduleid}`
        )
        .then((res) => {
      ...
      axios
        .get(`${process.env.REACT_APP_API_URL}quiz/getoldquestions/${moduleid}`)
        .then((res) => {
  ...
  const handleSubmit = () => {
    ...
    axios
      .post(`${process.env.REACT_APP_API_URL}quiz/updatequestionids`, {
        moduleid,
